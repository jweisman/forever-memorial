...
- **Media uploads**: allowed MIME types are split into `ALLOWED_IMAGE_TYPES` and `ALLOWED_VIDEO_TYPES` (combined as `ALLOWED_TYPES`) in `src/lib/s3-helpers.ts`; use `isVideoType(mimeType)` to branch logic. Image uploads get separate thumb + full presigned URLs. **Small videos** (≤ 50 MB) use a single presigned PUT URL. **Large videos** (> 50 MB, up to 500 MB) use S3 multipart upload via three endpoints: `POST /api/memorials/[id]/multipart-upload` (initiate + get part URLs), `.../multipart-upload/complete` (finalize with ETags), `.../multipart-upload/abort` (cleanup). Same pattern exists under `.../memories/[memoryId]/multipart-upload/`. Client-side orchestration is in `src/lib/upload.ts` (`uploadMultipartParts`). All uploads (images and videos) report progress via XHR `upload.onprogress` — pass `onProgress` callback to `uploadImage`/`uploadVideo`/`uploadMemoryImage`/`uploadMemoryVideo`. Update both MIME lists if adding new formats.
...
- **VIDEO**: `s3Key` is the direct file; `thumbUrl` and `url` are both the same presigned URL for that key; client-side Canvas resize is skipped; 500 MB limit (vs 15 MB for images); files > 50 MB use S3 multipart upload
...
8b. **mammoth pulls in jsdom (ESM-only)** — `mammoth` must be listed in `serverExternalPackages`. Its `jsdom` dependency uses ESM-only packages (`@exodus/bytes`) that cause `ERR_REQUIRE_ESM` when bundled. Current `serverExternalPackages`: `["pg", "pdfkit", "mammoth"]`. **Do not use `isomorphic-dompurify`** for the same reason — it also depends on jsdom v28+. Use `sanitize-html` instead (pure CJS, no jsdom). Life story sanitization uses `sanitize-html` in `PATCH /api/memorials/[id]`.
