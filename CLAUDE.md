...
8b. **mammoth and isomorphic-dompurify pull in jsdom (ESM-only)** — both must be listed in `serverExternalPackages`. Their shared `jsdom` dependency uses ESM-only packages (`@exodus/bytes`) that cause `ERR_REQUIRE_ESM` when bundled. Current `serverExternalPackages`: `["pg", "pdfkit", "mammoth", "isomorphic-dompurify"]`
