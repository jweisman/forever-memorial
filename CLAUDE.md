...
8b. **mammoth pulls in jsdom (ESM-only)** — `mammoth` must be listed in `serverExternalPackages`. Its `jsdom` dependency uses ESM-only packages (`@exodus/bytes`) that cause `ERR_REQUIRE_ESM` when bundled. Current `serverExternalPackages`: `["pg", "pdfkit", "mammoth"]`
